...
Hyperspectral Imaging (HSI) combines imaging and spectroscopy, assigning a continuous spectrum across wavelengths to each pixel. By capturing light-matter interactions—influenced by molecular composition and structure—HSI creates distinct spectral "fingerprints" for different tissues. While HSI is a valuable tool in medicine for tumor detection, publicly available hyperspectral tumor datasets are scarce. This scarcity often leads to overfitting in melanoma classification models, particularly for subsampled classes. Therefore, the main goal of this project was to construct a generative AI model capable of creating a synthetic hyperspectral dermoscopy dataset to augment limited real-world data. Specifically, we investigated whether a classifier trained with a combination of real and synthetic hyperspectral images outperforms one trained solely on real images or standard augmentations.

To test this hypothesis, we trained and evaluated multiple generative architectures—including SHSGAN, FastGAN, VAE, CycleGAN, and SPADE-FastGAN—comparing unconditional generation against strategies conditioned on RGB images. We assessed the quality of the output using spectral (SAM) and perceptual (FID, SSIM) metrics and integrated the synthetic data into the training pipelines of DenseNet201 and EfficientNet-b6 classifiers. Among the evaluated models, the unconditional FastGAN achieved the best balance between spectral accuracy and structural realism. Consequently, classifiers trained with mixed data (real plus synthetic) consistently outperformed baselines trained only on real data, with the optimal configuration increasing the Validation F1-score from 0.8852 to 0.9000 and specificity from 0.6429 to 0.7143.

Contrary to initial expectations, our experiments revealed that conditioning the generative models on RGB images did not yield superior classification results compared to unconditional generation. Furthermore, we observed no strong link between spectral fidelity and downstream performance, suggesting that current spectral metrics may not fully capture the nuances relevant for melanoma detection. While synthetic augmentation proved effective for improving sensitivity and validation metrics in data-scarce scenarios, challenges remain regarding generalization to independent test sets, highlighting the need for future research into spectral-aware loss functions to further capture the complexity of biological tissues.
