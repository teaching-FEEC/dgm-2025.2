...
class BasePredictorMixin:
    ...
    def _save_hyperspectral_sample(
        self,
        fake_img,
        batch_idx,
        i,
        ...
        savemat(
            os.path.join(output_dir, f"sample_{batch_idx}_{i:04d}.mat"),
            {"cube": fake_np},
        )

    def _save_rgb_sample(
        self,
        fake_img,
        batch_idx,
        i,
        ...
        Image.fromarray(rgb_uint8).save(
            os.path.join(output_dir, f"sample_{batch_idx}_{i:04d}.png")
        )

    def _get_sample_output_dir(
        self,
        base_output_dir: str,
        label: Optional[object],
    ) -> str:
        """
        Compute the output directory for a single sample.

        If `label` is provided (e.g., 0 or 1), use:
            base_output_dir/<label>/
        Otherwise, use:
            base_output_dir/
        """
        if label is None:
        ...
        # Normalize label to int for path construction.
        if isinstance(label, torch.Tensor):
            label = int(label.item())
        else:
            label = int(label)

        return os.path.join(base_output_dir, str(label))
    ...
    def _save_generated_batch(
        ...
        Args:
            fake_batch: Generated images, shape [B, C, H, W].
            batch_idx: Index of current batch.
            pred_hyperspectral: If True, save as .mat (hyperspectral).
                                If False, save as .png (RGB visualization).
            output_dir: Base directory for saving.
                        Defaults to hparams.pred_output_dir or
                        'generated_samples'.
            labels: Optional per-sample labels (e.g., 0 or 1) used to
                    route samples into subfolders:
                        output_dir/<label>/sample_*.mat|.png
                    Expected shape/len: B. Can be tensor, list, or np array.
        """
        base_output_dir = (
            output_dir
            or getattr(self.hparams, "pred_output_dir", "generated_samples")
        )
        gmin, gmax = self._prepare_global_minmax()

        # Convert labels to an indexable sequence if provided.
        label_seq = None
        if labels is not None:
            if isinstance(labels, torch.Tensor):
                ...
                # Handles list, tuple, numpy array, etc.
        ...
        for i in tqdm(range(fake_batch.size(0)), desc="Saving generated samples"):
            ...
            if label_seq is not None:
                # Assumes alignment between fake_batch and labels.
                label = label_seq[i]

            sample_output_dir = self._get_sample_output_dir(
                base_output_dir,
                label,
            )

            if pred_hyperspectral:
                self._save_hyperspectral_sample(
                    fake_img,
                    batch_idx,
                    i,
                    gmin,
    ...
    def on_load_checkpoint(self, checkpoint):
        ...
        - attempt to load state_dict non-strictly, removing keys that don't match
          this model (e.g. metric/net additions like 'mifid.*' that were removed
          from the codebase)
        """
    ...
    def load_state_dict(
        ...
        as long as this mixin is listed before LightningModule in the subclass
        MRO.
        """
        try:
