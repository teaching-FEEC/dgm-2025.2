...
class InceptionV3(nn.Module):
    ...
    def __init__(
        ...
        if use_fid_inception:
            inception = fid_inception_v3(in_chans=in_chans)
        else:
            inception = models.inception_v3(pretrained=True)
            if in_chans != 3:
                # Adapt first conv for non-RGB input
                first_conv = inception.Conv2d_1a_3x3.conv
                with torch.no_grad():
    ...
    def forward(self, inp):
        ...
        inp : torch.autograd.Variable
            Input tensor of shape BxCxHxW. Values are expected to be in
            range (0, 1)
...
def fid_inception_v3(in_chans=3):
    ...
    # Adapt first conv layer if in_chans != 3
    if in_chans != 3:
        # Get original Conv2d_1a_3x3 weights
        original_conv = inception.Conv2d_1a_3x3.conv
        adapted_weight = adapt_input_conv(in_chans, state_dict["Conv2d_1a_3x3.conv.weight"])

        # Create new conv layer with adapted channels
        new_conv = nn.Conv2d(
            in_chans,
        ...
        state_dict["Conv2d_1a_3x3.conv.weight"] = adapted_weight
