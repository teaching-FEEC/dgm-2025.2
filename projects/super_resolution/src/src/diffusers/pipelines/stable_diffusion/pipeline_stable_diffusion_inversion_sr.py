...
class StableDiffusionInvEnhancePipeline(
    ...
    def prepare_latents(
        ...
        if noise is None:
            ...
            # Ensure noise matches the latent shape (handle different scale factors)
            if noise.shape != shape:
                # Resize noise to match latents if shapes don't match
                if noise.shape[0] != shape[0]:
                    # Batch size mismatch - repeat or slice
                    if noise.shape[0] < shape[0]:
                        noise = noise.repeat((shape[0] // noise.shape[0]) + 1, 1, 1, 1)[:shape[0]]
                    else:
                        noise = noise[:shape[0]]
                # Resize spatial dimensions if needed
                if noise.shape[2:] != shape[2:]:
                    noise = torch.nn.functional.interpolate(
                        noise, size=shape[2:], mode='bilinear', align_corners=False
                    )
        ...
        **kwargs,
        ...
        if getattr(self, 'start_noise_predictor', None) is not None:
            with torch.amp.autocast('cuda'):
                ...
                # Resize noise to match the upscaled image's latent size
                # The noise predictor generates noise for the original image size,
                # but we need noise for the upscaled image's latent space
                if noise is not None:
                    # Compute expected latent size for upscaled image
                    latent_height = target_size[0] // self.vae_scale_factor
                    latent_width = target_size[1] // self.vae_scale_factor
                    expected_noise_shape = (noise.shape[0], noise.shape[1], latent_height, latent_width)
                    
                    # Resize noise if dimensions don't match
                    if noise.shape[2:] != (latent_height, latent_width):
                        noise = torch.nn.functional.interpolate(
                            noise, size=(latent_height, latent_width), mode='bilinear', align_corners=False
                        )
        else:
            noise = None
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if getattr(self, 'intermediate_noise_predictor', None) is not None and i + 1 < len(timesteps):
                    ...
                    # Resize noise to match the current latent size
                    if noise is not None and noise.shape[2:] != latents.shape[2:]:
                        noise = torch.nn.functional.interpolate(
                            noise, size=latents.shape[2:], mode='bilinear', align_corners=False
                        )
