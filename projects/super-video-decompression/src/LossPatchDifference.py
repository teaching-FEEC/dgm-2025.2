...
def patch_difference_loss(pred, target, source, upscale, patch_size=20, alpha=10):
    ...
    patch_loss = completeLoss(pred_patches_flat, target_patches_flat).mean(dim=(1, 2, 3))
    print("Patch Loss:", patch_loss.shape)

    # Normalize difference (avoid division by zero)
    print("Target Diff Max:", target_diff.max(dim=1, keepdim=True)[0])
    diff_norm = target_diff / (target_diff.max(dim=1, keepdim=True)[0] + 1e-8)
    print("Diff Norm Mean", diff_norm.mean())
   
    # Compute weighted loss
    weights = 1.0 + diff_norm
    #print(weights)
    loss = (weights * patch_loss).mean()
    print("Diff Loss Per Patch:",(weights * patch_loss).sum())
    
    print("Target Var Max:", target_var.max(dim=1, keepdim=True)[0])
    var_norm = target_var / (target_var.max(dim=1, keepdim=True)[0] + 1e-8)
    print("Var Norm Mean", var_norm.mean())
    print(diff_norm)
    print(var_norm)
    # Compute weighted loss
    weights = 1.0 + var_norm
    #print(weights)
    var_loss = (weights * patch_loss).mean()
    
    print("Var Loss Per Patch:",(weights * patch_loss).sum())
    ...
    print("Non WeightedLoss:", patch_loss.mean()* alpha)

    return loss * alpha,var_loss*alpha, variance_mask, diff_mask
