...
class TAB(nn.Module):
    def __init__(
        ...
        self.register_buffer("initted", torch.tensor(False))  # noqa: FBT003
    ...
    def forward(self, x):
        ...
        if not self.initted:
            ...
            x_means = torch.mean(
                rearrange(paded_x, "b (cnt n) c->cnt (b n) c", cnt=self.num_tokens),
                dim=-2,
            ).detach()
        else:
        ...
        if self.training:
            with torch.no_grad():
                ...
                if not self.initted:
                    ...
                    self.initted.data.copy_(torch.tensor(True))  # noqa: FBT003
                else:
    ...
    def __init__(
        ...
        if upscale == 4:
            ...
            self.upconv = nn.Conv2d(
                self.dim, self.dim * (upscale**2), 3, 1, 1, bias=True
            )
