...
def load_net():
    ...
    net = model.net_g
    print(net)
    print("PRETRAIN NETWORK",opt)
    modelPath = opt['path']['pretrain_network_g']
    print(modelPath)

    load_net = torch.load(
        modelPath, map_location=torch.device("cuda"), weights_only=True
    )
...
if __name__ == "__main__":
    ...
    rt = root_path
    opt,  args = parse_options(str(root_path), is_train=False)
    to_onnx()
